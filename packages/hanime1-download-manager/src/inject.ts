...
export function createInject(
  ...
  function injectWatchPage(): void {
    ...
    if (downloadBtn && !downloadBtn.dataset.h1dl) {
      ...
      if (videoId) {
        ...
        const labelContainer = icon
          ...
          : (downloadBtn.querySelector(".video-show-action-btn") as HTMLElement | null);
        if (labelContainer) {
    ...
    document.querySelectorAll("a.more-horiz-download-late").forEach((a: Element) => {
  ...
  function injectListings(): void {
    ...
    document.querySelectorAll(".video-item-container").forEach((card: Element) => {
